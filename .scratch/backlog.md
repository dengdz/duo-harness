...
- [ ] **自动抽取子代理（A 形态）**：turn 末后台抽取记忆的调度器——触发时机（turn 成功结束/空闲窗口）、噪声治理（抽取去重与置信门槛）、与显式写合并策略（A 抽取与 memory_write 的条目去重）三题待解。来源：M25 spec 决策三（可选增强挂账）；ZCode 探测 docs/research/ZCode/LLM与上下文/ 工单 10 ③。→ M26 规划时复议
