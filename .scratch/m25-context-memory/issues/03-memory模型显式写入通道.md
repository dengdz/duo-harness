...
## Status
in-progress（实现与自动化验证完成，待用户手动验收端到端演示）
...
- **待用户手动验收（done 前最后一步）**：
  1. 重启 CLI（运行Demo.md 命令块整块复制），`/new`；
  2. 输入：`记住：这个项目的记忆本功能是 M25 交付的`→ 预期 `[调工具] memory_write`（无审批弹卡）→ 确认语"已记入记忆本……"；
  3. `/new` 开新会话，输入：`这个项目的记忆本功能是什么时候交付的？`→ 预期模型直接答出（不调工具），证明写入跨会话生效；
  4. 同时验证读路径仍好使：问"我的记忆本里记了什么"应直接引用注入段；
  5. 通过后工单转 done。
