...
### BUG-20260925-02 · 记忆注入生效但模型首轮先 read 文件（guide 缺指令性）（2026-09-25，verifying：待用户复验采信层）
...
- **修复**：GUIDE_TEXT 增指令句（直接引用本轮注入段、无需读文件）+ header 锚定"本段为本轮最新内容"。
- **防复发**：采信层 mock 锁不住——用户复验一条命令（问句不出现 `[调工具] read`）；"提示词契约写指令形态不写信息形态"落 duo-workflow experience.md。
