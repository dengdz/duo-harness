...
- [x] 新增 `permission/mode` 会话事件（latest-wins 投影，plan/mode 同款先例）：/permission 切档时落事件；会话打开投影恢复最后档位（写回 WorkspacePolicy）；新会话/新部署回 yml 缺省（档位跟对话走，不跨会话惊吓）——**落地注记（BUG-20260919-03）**：恢复时机分档，占用被迫改开的新会话继承被占会话档位并落继承事件；显式换绑无记录才重置缺省
