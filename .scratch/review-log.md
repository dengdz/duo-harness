...
### 2026-09-24 · C1-02 审查（main 工作树，四轴制：duo 侧对比报告 + 讲义）

- **范围与轮次**：四轴（Standards / Spec 新上下文子代理并行；行级/Java 轴按定义空集执行；基点 = 工作树 vs HEAD）→ 修复 → docs:build 收口。
- **计数**：Standards 6 + Spec 5（重叠 2）→ 合并 9 处。处置：修 9 / 记档 0。零阻断；锚点抽验 30+ 全中。
- **最重的发现**：三组统计错误两轴独立复算正中——①中位数取错位（15 值第 9 位当第 8 位，77→68）；②矩阵计数与表内标记矛盾（◐ 表内 9 项结论写 7 项）；③枚举不全（三级外链 7 处记 2 处）。全部属于 C1-01 已固化的「计数落笔重数」教训家族——本轮 brief 已把三条教训写进 Standards 轴规则清单，两轴据此主动复算并抓回，教训前置派发有效。
- **模式化问题（本次新识别）**：
  1. **「结论句」是独立命题，不只表体要核**——矩阵表内标 ◐/✗ 正确、结论句却数错，说明检查清单要单列一条「表格/列表与其上方结论句互核」；同类：同格内"最长 138"与"7–140 行"自相矛盾。
  2. **引文拼接是最隐蔽的失真形态**——把 invocation.md:5 与 :8 两个半句拼成一个"原文"加一个锚点，读起来完全成立；引文要么逐字单出处，要么两处分别直引。
  3. **修正背景线索时，修正值本身也要复算**——§4"约 70→实测 77"是把 24 版对的数字改错了；对二手结论做修正前，一手数据先独立算一遍并保留中间序列（排序表）。
- **收口**：`npm run docs:build` 通过（1.21s）；零 Java 触碰；报告全文并入 `.scratch/c1-skill-writing/issues/02-duo侧对照与对比报告讲义.md` 审查轮小节。
