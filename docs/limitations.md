...
| # | 限制 | 说明与去向 |
|---|---|---|
| 1 | 计划模式为引导式，不硬禁工具 | 激活时靠计划指导片段约束行为；写操作的真实防线是交互审批（M6）。硬禁需权限预设 / 沙箱（M9+） |
| 2 | 技能无热加载 | 仅启动扫描，技能目录变更需重启进程（M9+ 候选） |
| 3 | AGENTS.md 注入为最小链 | 仅用户全局 + 项目根两文件；嵌套子目录链与 fs 操作后增量发现未做（M9+ 候选） |
| 4 | 思考内容回传的 provider 间歇校验 | 思考模型 + 多轮工具链下，provider 对 reasoning_content 回传的校验呈间歇性（诊断三连复现困难）；已按"随事件持久化、投影全量回传"根治（BUG-20260913-03），若再遇 400 需捕获失败请求体上报 |
