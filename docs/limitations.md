...
| 5 | 工具域强化件部分缺位 | output schema 契约校验、guard、并发安全分类、scope 遮蔽未实现——M2 工具域强化交付（审批策略已落地） |
| 6 | Web 双面 / agent 循环未开始；MCP output 契约未接线 | M2 进行中（MCP 连接与远端工具同步、工具域审批策略已落地；审批与 MCP 工具的端到端接线见工单 05）；M3（页面）/ M4（agent+LLM）见 ADR-0004 路线图 |
