...
| 2 | CLI 与 Web 并发写同一会话未协调 | 约定单入口使用（同一时刻只用一个入口）；多端并发写入的仲裁属后续里程碑 |
| 3 | 界面为无构建静态单页 | 原生 JS 无框架——交互复杂度上限受控；组件化 / 构建链在界面规模化时再引入（M9+） |
| 4 | 多浏览器标签共用一个"服务端当前会话" | /new 与切换会话改的是服务端全局状态，多标签同时操作会互相改写对方的当前会话（BUG-20260914-01 实测）；单标签使用不受影响，多会话并行属后续里程碑 |
