...
| 输入框发消息 | 发送按钮受理转「思考中…」禁用，执行中**同钮转【停止】**（危险色，点按协作式中断——已流出内容保留、中断标记可见、再发消息即续接；完成后回【发送】）→ 用户气泡唯一 → 工具合一卡（⟳ 运行中 → ✓ 成功/✗ 失败，结果可折叠）→ 流式回复，完成后 Markdown 整段渲染（代码块/列表/标题）；首条消息后侧栏与标签页自动出现会话标题（异步生成，失败降级首条前 20 字）；执行中回车发送 = 注入插队（与 CLI 同语义） |
