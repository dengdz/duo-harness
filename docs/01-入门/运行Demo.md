...
> 状态：M1 + M2 一条命令演示全链路可用；M3 聊天 REPL、M5 工具循环 REPL、M6 人机协同（HITL）、M7 技能与计划模式均可交互运行（见下文）。
...
同一条命令（agent-demo.yml 已含 web 行），启动时控制台打印 `Web 面已启动: http://127.0.0.1:8080`——浏览器打开即用：

| 操作 | 预期 |
|---|---|
| 打开页面 | 双区布局：左对话（续接最新会话，历史回放）/ 右状态（插件六态表 + 工具清单） |
| 输入框发消息 | 用户消息上屏 → 工具合一卡（⟳ 运行中 → ✓ 成功/✗ 失败）→ 流式回复 |
| 触发写操作 | 审批按钮卡片 → 点批准（执行）/ 拒绝（模型解释） |
| 关闭页面 | 悬空审批自动拒绝（fail-closed），重开可见"未获批准" |
| CLI 并存 | 终端 AgentRepl 照常可用（同一会话目录；约定单入口使用） |

安全基线：只绑 127.0.0.1（局域网不可访问），无鉴权（M9+ 按需）。
