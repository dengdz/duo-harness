...
- **memory 记忆本写路径（M25 工单 03，ADR-0024 M25 节）**：新增 `memory_write` 工具——会话中说"记住 X"，模型把内容追加进项目记忆本 `.duo/MEMORY.md`，之后的每轮请求（含新会话）自动注入生效；追加式写入在服务端保证用户手改不被模型覆写吞掉（模型没有整文件覆写面），写入经工具管线落 tool/call + tool/result 会话事件可回放可审计；工具在册时 memory-guide 规范段带写协议下发（教模型何时写、如何去重、改删引导用户手改文件），tools 行缺席的纯对话装配只保留读路径零感降级。记忆本"会话中记住 → 下会话不用重复交代"闭环交付

- **memory 记忆本读路径（M25 工单 02，ADR-0024 M25 节）**：项目级 `.duo/MEMORY.md`（个人记忆，已入 .gitignore 不入库）作为跨会话记忆本——每轮模型请求现读其内容，包 `<memory>` 标签附时效免责语以 user 角色置于消息序列最前（meta_user 通道，请求视图专用不落会话日志，先于治理投影组装不被压缩吞）；注入带 16KB 预算上限（`budgetChars` 可配），超限截尾并标注；`memory-guide` 规范段随记忆本在场注册进 system（告诉模型记忆段语义与时效边界，写协议随工单 03）；文件缺席/为空/读取异常静默降级为零注入——未启用用户零感知。手写一条记忆进 MEMORY.md，开新会话 AI 即"知道"，无需任何操作
...
- **M25（0.20.0）启动规划落盘：grill 五问收敛 + spec 与 8 张工单**：主题按 ADR-0024 既定「上下文与记忆——长会话记得住」三件全做——①memory 域最小形态（项目级 `.duo/MEMORY.md` 不入 git、会话中模型显式写 + meta_user 注入带预算、不存在静默降级；自动抽取 A 作可选增强挂账）②microcompact 免模型本地裁剪（保最近 N 组 + 媒体/错误豁免 + 裁剪事件留痕）+ 压缩熔断计数 + summary 预算隔离 ③cacheControl 三级断点（身份前缀/稳定身份/动态段，不支持 provider 静默降级 + 用量透出）+ AGENTS.md 迁 meta_user 与嵌套子目录链（M7#3 搭车销账）。开工手续 = 工单 01（0.19.0 盖章发布打 tag + 切 0.20.0 分支）；依赖形状 = 01 后三路并行（02/04/06）、03/05/07 各自接力、08 收尾。spec `.scratch/m25-context-memory/spec.md`（20 条用户故事 + 裁剪破坏性/缓存降级两条风险记档）；术语表三词条（memory/microcompact/cacheControl）随实现同 diff；M25 起写法受《技能写作规范》与红线 9 约束（C1 交付兑现）
