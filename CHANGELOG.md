...
- 会话事件 `run/error`（M8）：运行错误直推帧（不落会话历史），页面渲染 [错误] 卡`WebPlugin`（Boot yml 一行，只绑 127.0.0.1，默认 8080）+ 静态单页（对话/状态双区，亮色 DSH 风格）+ `/api/status` 状态 JSON + `/api/events` SSE 会话事件流（存量回放 + 实时推送，虚拟线程执行器）
