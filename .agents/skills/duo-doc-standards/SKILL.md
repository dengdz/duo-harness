...
description: 文档位置与结构标准：docs/ 章节归属、教程与参考体裁、全库文档审计（泄漏/重复/对账）。触发：这个文档放哪 / 整理文档 / 文档太长 / 审计文档。
---

# 应用 duo-harness 文档规范

文档规则活在 `docs/index.md`（建立后）的导航结构与各篇自身。本技能覆盖放置、结构分类、全库文档审计与验证。**它是指导，不是脚本**；编辑判断（该保留什么、删什么）：Call the Skill tool with "duo-trim-cot-leakage"；**永远不要把长度本身当缺陷**。质量与写作规范：Call the Skill tool with "duo-prose-standard"。
...
2. **猎捕思维链泄漏** —— Call the Skill tool with "duo-trim-cot-leakage"：变更叙述、死引用、审查过程记录、流程复述。文档里最常见的形态："如前所述"、审查轮次引用、"本次重构后"、"状态更新"字样（变更叙述的变体）、本地工单/issue 编号引用（编号指向 `.scratch` 等本地档案，仓库读者不可达）。
...
6. **导航对账**（文档站专检）：`config.mts` 的 nav/sidebar 条目与 `docs/` 实际 md 文件双向对账——docs/ 下有文件而 sidebar 无条目 = 漏同步（导航不可见）；sidebar 有条目而文件不存在 = 死链。重点核对 adr/ 目录：ADR 文件数应等于 sidebar"决策记录"分组的条目数。push main（触发部署）前必查。
   ...
   ```
   两数相等 = N 份 ADR + nav 顶层 1 条 `/adr/` 入口，即 grep 数应比文件数**多 1**（N 随 ADR 增长，写死具体数会过期——判据是差值不是 N）。
   完成判据：两数已出示，差值 = 1。
