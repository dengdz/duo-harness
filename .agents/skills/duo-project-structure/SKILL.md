...
description: 分层目录结构标准：Maven 模块、Java 包组织、文件归属、拆包时机、结构评审。触发：新建模块 / 建个包 / 这个类放哪 / 搭目录结构 / 结构评审。
---
...
- **每个包必须有 `package-info.java`**：一句话职责 + 主要类型说明（写作规范：Call the Skill tool with "duo-prose-standard"）。
...
用户要求评审目录结构时，按层收集事实再下结论，避免只数文件。范围不明时先向用户确认评审边界（fail-fast：模块、包、还是全仓），不自行圈定。
