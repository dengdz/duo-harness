...
**问题描述**：
memory 注入验收首问，模型在有注入段的情况下仍先 read 文件再回答——注入"送达"了但模型没"采信"；guide 写的是"内容会以 <memory> 段注入"（信息性），header 的"可能过时"免责语还被模型反向归因（拿历史过期的 read 结果质疑本轮新鲜注入段）。

**原因分析**：
描述性提示不构成模型行为约束——"X 会发生"只建立预期，不约束"遇到 X 该怎么做"；免责语没有锚定参照点时，模型可能把时效性归因到错误一侧。

**解决方案**：
面向模型的契约一律写成指令形态："回答 X 类问题时直接引用 Y，无需 Z（除非用户明确要求）"；时效/权威语锚定参照点（"本段为本轮最新内容"）。验证分层：送达层 mock 可锁（断言请求视图），采信层只能真实模型验收（判据 = 不再出现对应工具调用）。
