...
Java 实现的插件化 AI agent harness：内核是自研轻量插件容器，所有能力（工具 / MCP / LLM / 会话与上下文治理 / 技能 / 计划模式 / 人机协同 / 子代理 / hooks）以插件形式组装。已发布 0.1.0-0.14.0（扩展机制：hooks 生态兼容、插件可选依赖、awaitStartup 超时，及并发工具调度、崩溃恢复、入口栅栏等加固；0.14.0：斜杠命令与运行中治理——命令注册表、Web 斜杠入口、/compact 压缩点、呈现位亲和路由、父级 steer、还账三件）。
