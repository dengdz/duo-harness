...
Java 实现的插件化 AI agent harness：内核是自研轻量插件容器，所有能力（工具 / MCP / LLM / 会话与上下文治理 / 技能 / 计划模式 / 人机协同 / 子代理）以插件形式组装。已发布 0.1.0-0.10.0（CI 门禁、入口栅栏、审批钉死、崩溃恢复、流式空闲超时等加固随 0.11.0 收口中）。
...
| `duo-harness-tools` | 工具域：注册与三段执行管线，审批 / guard / 输出契约治理挂链生效；本机 fs 工具族与三档权限预设 |
...
| `duo-harness-agent` | agent 编排：工具循环（Function Calling 闭环）、prompt 注册表、技能系统、计划模式、AGENTS.md 注入、上下文治理、子代理任务分解（spawn/fork + 控制面） |
